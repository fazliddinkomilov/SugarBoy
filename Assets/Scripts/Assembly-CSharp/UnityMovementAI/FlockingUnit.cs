...
namespace UnityMovementAI
{
	public class FlockingUnit : MonoBehaviour
	{
		...
		private void FixedUpdate()
		{
			...
			linearAcceleration += cohesion.GetSteering(sensor.targets) * cohesionWeight;
			linearAcceleration += separation.GetSteering(sensor.targets) * separationWeight;
			linearAcceleration += velocityMatch.GetSteering(sensor.targets) * velocityMatchWeight;
			if (linearAcceleration.magnitude < 0.005f)
			{
				linearAcceleration = wander.GetSteering();
			}
