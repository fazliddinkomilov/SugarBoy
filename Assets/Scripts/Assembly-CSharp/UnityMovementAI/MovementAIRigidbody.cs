...
namespace UnityMovementAI
{
	public class MovementAIRigidbody : MonoBehaviour
	{
		...
		private IEnumerator DebugDraw()
		{
			...
			Debug.DrawLine(colliderPosition, colliderPosition + Velocity.normalized, Color.red, 0f, depthTest: false);
			if (is3D)
			{
				Debug.DrawLine(colliderPosition, colliderPosition + RealVelocity.normalized, Color.green, 0f, depthTest: false);
				Debug.DrawLine(colliderPosition, colliderPosition + wallNormal, Color.yellow, 0f, depthTest: false);
				Debug.DrawLine(colliderPosition, colliderPosition + movementNormal, Color.blue, 0f, depthTest: false);
			}
			count++;
			countDebug = 0;
			StartCoroutine(DebugDraw());
		...
		private void FixedUpdate()
		{
			...
			if (num && SphereCast(Vector3.down, out var hitInfo, groundFollowDistance, groundCheckMask.value))
			{
				if (IsWall(hitInfo.normal))
				{
					...
					if (num2 > 0f && SphereCast(normalized, out var hitInfo2, num2, groundCheckMask.value) && !IsWall(hitInfo2.normal))
					{
		...
		private void LimitMovementOnSteepSlopes()
		{
			...
			for (int i = 0; i < 2; i++)
			{
				...
				{
				case 0:
					Debug.DrawRay(colliderPosition + Vector3.up * 0.05f * countDebug, normalized, new Color(0.953f, 0.898f, 0.961f), 0f, depthTest: false);
					break;
				case 1:
					Debug.DrawRay(colliderPosition + Vector3.up * 0.05f * countDebug, normalized, new Color(0.612f, 0.153f, 0.69f), 0f, depthTest: false);
					break;
				default:
					Debug.DrawRay(colliderPosition + Vector3.up * 0.05f * countDebug, normalized, new Color(0.29f, 0.078f, 0.549f), 0f, depthTest: false);
					break;
				}
				if (!SphereCast(normalized, out var hitInfo, dist, groundCheckMask.value, movementNormal) || !IsWall(hitInfo.normal) || !IsMovingInto(normalized, hitInfo.normal))
				{
					break;
				}
				float num = Mathf.Max(0f, hitInfo.distance);
				rb3D.MovePosition(rb3D.position + normalized * num);
		...
		private Vector3 LimitVelocityOnWall(Vector3 velocity, Vector3 planeNormal)
		{
			...
			else
			{
				...
				Vector3 velocity2 = rb3D.velocity;
				velocity2.y = 0f;
				velocity2 = Vector3.ProjectOnPlane(velocity2, planeNormal);
				if (Vector3.Angle(from, velocity2) > 90f)
				{
					velocity2 = Vector3.Project(velocity2, vector);
				}
				velocity2.y = y;
				velocity2 = Vector3.ProjectOnPlane(velocity2, planeNormal);
				velocity = velocity2;
